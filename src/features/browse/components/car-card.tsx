...
const BrowseCarCard = ({ car }: BrowseCarCardProps) => {
    ...
    const mileageInEnglishDigits = new Intl.NumberFormat("en-US").format(car.mileage_km);
    const details = [car.fuel_type, car.car_type || "-", `${mileageInEnglishDigits} ${t("km")}`, String(car.car.year)].join(" | ");
    ...
    const whatsappMessage = encodeURIComponent(`${car.car.brand} ${car.car.model} - ${formatPrice(Number(car.price), i18n.language)} ${t("EGP")}`);
    ...
    return (
        <article className="overflow-hidden rounded-xl border border-stone-200 bg-white shadow-sm transition-shadow hover:shadow-md">
            ...
            <div className="space-y-3 p-4">
                <Link to={`/cars/${car.id}`} className="block">
                    <h3 className="line-clamp-1 text-xl font-bold text-stone-900">
                        {car.car.brand} {car.car.model}
                    </h3>
                </Link>

                <p className="line-clamp-1 text-sm text-stone-600">{details}</p>

                <div className="text-2xl font-extrabold leading-none text-primary">
                    {formatPrice(Number(car.price), i18n.language)}
                    <span className="ms-1 text-xl">{t("EGP")}</span>
                </div>

                <p className="line-clamp-1 text-sm text-stone-500">
                    <HiOutlineLocationMarker className="me-1 inline" size={16} />
                    {locationLine}
                </p>
                ...
                <div className="grid grid-cols-2 gap-2 pt-1">
                    <a
                        ...
                        className={`inline-flex items-center justify-center gap-2 rounded-md border border-stone-300 px-3 py-2 text-sm font-semibold transition-colors ${
                            hasPhone ? "bg-white text-stone-700 hover:bg-stone-100" : "cursor-not-allowed bg-stone-100 text-stone-400"
                        }`}
