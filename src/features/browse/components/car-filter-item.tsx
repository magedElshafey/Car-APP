...
const FilterItemContext = createContext<FilterItemContextType | undefined>(
  undefined,
);
...
const FilterItem: FC<PropsWithChildren<FilterItemProps>> & {
  Header: typeof FilterItemHeader;
  Menu: typeof FilterItemMenuContainer;
} = ({ children, filterKey, className = "" }) => {
  ...
  return (
    <FilterItemContext.Provider
      value={{
        isOpen: state,
        toggle,
        filterValue: filterKey ? states[filterKey] : undefined,
        filterKey,
      }}
    >
...
export const FilterItemHeader: FC<FilterItemHeaderProps> = ({
  ...
  const isLoading =
    showLabel && isStringFilterValue(filterValue) && !label && filterValue.name;
  return (
    <div
      onClick={toggle}
    ...
    >
      ...
      {isLoading ? (
        <AiOutlineLoading3Quarters
          size={12}
          className="animate-spin text-stone-800"
        />
      ) : label && showLabel ? (
        <span className="overflow-hidden text-sm text-gray-500 max-w-1/2 text-ellipsis whitespace-nowrap">
          {label}
        </span>
      ) : (
        <FaAngleDown
          className={`text-gray-500 duration-100 transition-all ${isOpen ? "rotate-180" : "rotation-0"}`}
        />
      )}
    </div>
  );
...
export const FilterItemMenuContainer: FC<
  PropsWithChildren<{ searchable?: boolean }>
> = ({ children, searchable = false }) => {
  ...
  return (
    ...
    >
      <div
        className={`space-y-2 flex flex-col overflow-hidden border-b border-b-stone-300 transition-all duration-200 ${isOpen ? "max-h-96 pb-2" : "max-h-0"}`}
      >
        {searchable && (
          <div className="flex items-center p-1 border rounded-md group/search border-stone-200 focus-within:border-blue-500">
            <input
              className="flex-1 outline-none appearance-none"
              value={search}
              onChange={(e) => handleChange(e.target.value)}
              placeholder={
                filterKey ? t(`browse.filters.search.${filterKey}`) : ""
              }
            />
            <HiMagnifyingGlass className="text-stone-200 group-focus-within/search:text-blue-500" />
          </div>
        )}
        <div className="flex-1 overflow-y-auto filter-item-menu">
          {children}
        </div>
      </div>
    </SearchContext.Provider>
...
function getFilterLabel(
  filterKey: keyof Filters | undefined,
