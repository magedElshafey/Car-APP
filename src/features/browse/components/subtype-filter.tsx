...
const SubtypeFilterMenu = () => {
  ...
  useEffect(() => {
    if (subtypes && sub_type?.value && !sub_type.label) {
      const active = subtypes.find(
        (b) => b.value.toString() === sub_type.value,
      );
      if (active) {
  ...
  if (subtypesLoading || !activeFilters.vehicle_type)
    ...
    <div className="p-1 rounded-sm bg-slate-100">
      {!!filteredModels?.length ? (
        filteredModels?.map((type) => (
          <button
            className={`py-2 block w-full text-start rounded px-2 duration-75 cursor-pointer font-semibold text-sm ${activeSubtype === type.value ? "text-blue-400 bg-blue-50" : "hover:bg-slate-300"}`}
            key={type.value}
            onClick={() => {
