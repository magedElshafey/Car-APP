...
import { isRangeFilterValue, type RangeFilterKey } from "../types/filters.types";
...
const RangeFilter = ({
    ...
    const activeValue = isRangeFilterValue(states[filterKey]) ? states[filterKey] : undefined;
        ...
        <FilterItem filterKey={filterKey}>
            ...
            <FilterItem.Menu>
                <DoubleSlider
                    ...
                    value={{ min, max }}
                    onChange={(value) => {
                        const isDefaultRange = value.min === config.min && value.max === config.max;
