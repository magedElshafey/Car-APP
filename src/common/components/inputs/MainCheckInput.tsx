...
interface MainCheckInputProps extends Omit<
    ...
    return (
      <div className="flex flex-col gap-1">
        {labelPosition === "above" && label && (
          <label
            htmlFor={inputId}
            className={`text-sm md:text-base block mb-2  text-muted ${
              disabled ? "opacity-50" : ""
            }`}
          >
        ...
        >
          ...
          {labelPosition === "inline" && label && (
            <span
              className={`text-sm md:text-base font-medium text-muted ${
                disabled ? "opacity-50" : ""
              }`}
            >
              {typeof label === "string" ? t(label) : label}
        ...
        <div
          className={`transition-all duration-100 min-h-2 mt-1 overflow-hidden ${
